...
const { data: eventData, pending, error, refresh } = await useLazyAsyncData('event', async () => {
	try {
    const prismicCode = locale.value === 'ua' ? 'uk-ua' : 'en-us';
		const event = await $prismic.client.getByUID('event', route.params.event_uid, { lang: prismicCode });
